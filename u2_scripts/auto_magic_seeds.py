...
def env_ratio(name, default=2.33):
    return max(1.3, min(2.33, env_float(name, default)))


def parse_qb_clients():
    raw = os.environ.get('QB_TARGETS', '').strip()
    if raw:
        targets = json.loads(raw)
    else:
        targets = [{
            'host': os.environ.get('QB_HOST', 'http://127.0.0.1'),
...
CONFIG = {  # 应该跟 json 差不多，放到 ide 里方便能看出错误
    'clients_info': parse_qb_clients(),
    'requests_args': {
        'cookies': {'nexusphp_u2': os.environ.get('U2_COOKIE', '')},  # 网站 cookie
        'headers': {
    ...
    'magic_for_self': {  # 做种中的种子，上传速度大于一定值，给自己放 2.33x 魔法
        'enable': env_bool('MAGIC_SELF_ENABLE', True),  # 是否开启
        'interval': env_int('MAGIC_SELF_INTERVAL', 60),  # 检查的间隔
        'magic_downloading': env_bool('MAGIC_DOWNLOADING', True),  # 是否下载中的种子放 2.33x 魔法
        'upload_ratio': env_ratio('MAGIC_UPLOAD_RATIO', 2.33),  # 上传魔法倍率，范围 1.3 ~ 2.33
        'existing_skip_ratio': env_ratio('MAGIC_EXISTING_SKIP_RATIO', 2.0),  # 已有上传倍率达到此值就不再放
        'min_rate': env_int('MAGIC_MIN_RATE', 1024),  # 最小上传速度(KiB/s)
        'min_size': env_float('MAGIC_MIN_SIZE', 5),  # 最小体积(GiB)
        'min_d': env_float('MAGIC_MIN_DAYS', 180),  # 种子最小生存天数
    },
    'magic_for_all': {  # 做种中的种子，做种人数小于一定值，放地图炮 free，吸引别人下载
        'enable': env_bool('MAGIC_ALL_ENABLE', False),  # 是否开启
        'interval': env_int('MAGIC_ALL_INTERVAL', 86400),  # 检查的间隔
        'torrent_num': env_int('MAGIC_ALL_TORRENT_NUM', 5),  # 一次放魔法的种子个数
        'max_seeder_num': env_int('MAGIC_ALL_MAX_SEEDER_NUM', 5),  # 做种人数最大值
        '233_all': env_bool('MAGIC_ALL_233_ALL', True),  # 为真时给所有人放 2.33x↑0x↓，否则给所有人 0x↓，自己放 2.33x↑
        'hours': env_int('MAGIC_ALL_HOURS', 24),  # 魔法持续时间
        'min_rm_hr': env_float('MAGIC_ALL_MIN_RM_HR', 0)  # 2.33x 剩余时间小于这个值(小时)还是会放 2.33x 魔法
    },
    'uc_max': env_int('UC_MAX', 30000),  # 单个魔法最大 uc 使用量
    'total_uc_max': env_int('TOTAL_UC_MAX', 200000),  # 24h 内 uc 最大使用量
    'api_token': os.environ.get('U2_API_TOKEN', ''),  # api 的 token，填了将默认使用 api 查询种子信息，不填就直接从 u2 网页获取信息
    'uid': env_int('U2_UID', 0),  # 如果填了 api_token，则需要 uid
    'data_path': os.environ.get(
        'AUTO_MAGIC_DATA_PATH', f'{os.path.splitext(__file__)[0]}.data.txt'
    ),  # 数据保存路径
    'log_path': os.environ.get(
        'AUTO_MAGIC_LOG_PATH', f'{os.path.splitext(__file__)[0]}.log'
    ),  # 日志路径
}
...
class Qbittorrent(qbittorrentapi.Client, BtClient):
    ...
    def fix_return_value(self, lst, keys):
        torrents_info = {}
        if not lst:
            logger.warning(f'No torrent data returned from qbittorrent on {self.host}:{self.port}, skipped')
...
class MagicSeed(Request):
    ...
    async def main(self):
        ...
        with self.magic_lock:
            for data in self.magic_info.values():
                if 'tid' in data and ('ts' in data or 'reserved' in data):
                    self.reserved_tids.add(str(data['tid']))
        ...
        with self.magic_lock:
            for __id, _tid, ur_233 in res:
                tid_key = str(_tid)
                if _tid and __id not in self.magic_info and tid_key not in self.reserved_tids:
                    self.reserved_tids.add(tid_key)
                    self.magic_info[__id] = {'reserved': int(time()), 'tid': _tid}
                    magic_tasks.append(self.send_magic(
                        __id,
                        _tid,
                        {'user': 'SELF', 'hours': 24, 'ur': CONFIG['magic_for_self']['upload_ratio'], 'dr': 1}
                    ))
        async with aiohttp.ClientSession() as self.session:
    ...
    async def info_from_u2(self, _id, name):
        ...
        if table:
            ...
            for img in cont[1].select('tr')[1].td.select('img') or []:
                existing_ratio = float(img.next_element.text[:-1].replace(',', '.'))
                if img.get('class') == ['arrowup'] and existing_ratio >= CONFIG['magic_for_self']['existing_skip_ratio']:
                    logger.info(
                        f"Torrent {_id}, id: {tid}: {existing_ratio}x upload magic existed!"
                    )
                    time_tag = cont[1].time
                    if time_tag:
                        magicst = max(int(time()), int(self.ts(time_tag.get('title') or time_tag.text, timezone) - 86400))
                        self.magic_info[_id] = {'ts': magicst, 'tid': tid}
                    else:
                        self.magic_info[_id] = {'ts': int(time()) + 86400 * 30, 'tid': tid}
                    return _id, tid, True

            # 判断种子时间是否小于最小天数
            delta = time() - self.ts(cont[3].time.get('title') or cont[3].time.get_text(' '), timezone)
            if delta < CONFIG['magic_for_self']['min_d'] * 86400:
                self.magic_info[_id] = {'ts': int(time()), 'tid': tid}
    ...
    async def info_from_api(self, _id, name):
        ...
        if history_data['data']['history']:
            ...
            existing_ratio = float(res['data']['promotion_super'][0]['private_ratio'].split(' / ')[0])
            if existing_ratio >= CONFIG['magic_for_self']['existing_skip_ratio']:
                logger.info(
                    f"Torrent {_id}, id: {tid}: {existing_ratio}x upload magic existed!"
                )
                ...
                for pro_data in pro_list:
                    if float(pro_data['ratio'].split(' / ')[0]) >= CONFIG['magic_for_self']['existing_skip_ratio']:
                ...
                self.magic_info[_id] = {'ts': max(int(time()), int(pro_end_time) - 86400), 'tid': tid}
                return _id, tid, True
    ...
    async def send_magic(self, _id, tid, _data):
        ...
        try:
            if 'ts' in self.magic_info.get(_id, {}) and int(time()) - self.magic_info[_id]['ts'] < 86400:
                logger.info(f'Torrent {_id}, id: {tid}: magic already recorded, skipped.')
                return
            ...
            if res_json['status'] == 'operational':
                ...
                if cost > CONFIG['total_uc_max']:
                    ...
                    return
                self.magic_info[_id] = {'uc': uc, 'tid': tid}
    ...
    def __init__(self):
        ...
        with open(CONFIG['data_path'], 'r', encoding='utf-8') as fp:
            try:
                ...
                pass
        now = int(time())
        for info in self.magic_info.values():
            if 'tid' in info and 'ts' in info and now - info['ts'] >= 86400:
                info['ts'] = now
                self.magic_info.c = True
        self.magic_info.save()
